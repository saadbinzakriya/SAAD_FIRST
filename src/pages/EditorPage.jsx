...
import {
  GlobalStyle, ThemeSwitcher, THEMES, themeVars, CustomCursor, getSystemTheme,
  FontSwitcher, fontVars,
  useReducedMotion, useIsTouch,
  ProfileTab, SkillsTab, ExperienceTab, ProjectsTab,
} from '../portfolio/lib.jsx';
const CVDownloadButtons = lazy(() => import('../cv/CVDownloadButtons.jsx'));
...
export default function EditorPage() {
  ...
  const [fontKey, setFontKeyState] = useState('grotesk');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setPortfolio(data);
      setThemeKeyState(data.theme || getSystemTheme());
      setFontKeyState(data.font || 'grotesk');
      try {
  ...
  const changeFont = async (key) => {
    setFontKeyState(key);
    await supabase.from('portfolios').update({ font: key, updated_at: new Date().toISOString() }).eq('slug', slug);
  };
  ...
  if (!authed) {
    return (
      <div className="mecha-root no-native-cursor" style={{ ...themeVars(theme), ...fontVars(fontKey) }}>
        <GlobalStyle />
        <CustomCursor enabled={cursorEnabled} />
        <ThemeSwitcher themeKey={themeKey} setThemeKey={changeTheme} />
        <FontSwitcher fontKey={fontKey} setFontKey={changeFont} />
  ...
  return (
    <div className="mecha-root no-native-cursor" style={{ minHeight: '100vh', ...themeVars(theme), ...fontVars(fontKey) }}>
      <GlobalStyle />
      <CustomCursor enabled={cursorEnabled} />
      <ThemeSwitcher themeKey={themeKey} setThemeKey={changeTheme} />
      <FontSwitcher fontKey={fontKey} setFontKey={changeFont} />
      ...
      <div className="px-6 pb-4">
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <Suspense fallback={<div className="panel-2 rounded-lg p-5 text-muted" style={{ fontSize: 12.5 }}>Loading CV tools...</div>}>
            <CVDownloadButtons profile={portfolio.profile} skills={portfolio.skills} experience={portfolio.experience} projects={portfolio.projects} />
          </Suspense>
        </div>
      </div>
