...
import {
  GlobalStyle, ThemeSwitcher, THEMES, themeVars, FontSwitcher, fontVars, getSystemTheme,
...
export default function PublicPortfolioPage() {
  ...
  const [fontKey, setFontKey] = useState('grotesk');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setPortfolio(data);
      setThemeKey(data.theme || getSystemTheme());
      setFontKey(data.font || 'grotesk');
      setLoading(false);
    })();
  ...
  return (
    <div className={`mecha-root ${cursorEnabled ? 'no-native-cursor' : ''}`} ref={rootRef} style={{ minHeight: '100vh', ...themeVars(theme), ...fontVars(fontKey) }}>
      <GlobalStyle />
      <CustomCursor enabled={cursorEnabled} />
      <ThemeSwitcher themeKey={themeKey} setThemeKey={setThemeKey} />
      <FontSwitcher fontKey={fontKey} setFontKey={setFontKey} />
      <ScrollProgressBar />
      ...
      <div className="px-6 pb-16" style={{ maxWidth: 640, margin: '0 auto' }}>
        <Suspense fallback={null}>
          <CVDownloadButtons profile={profile} skills={skills} experience={experience} projects={projects} />
        </Suspense>
      </div>
