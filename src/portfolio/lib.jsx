...
export function svgPlaceholder(label) {
  ...
  const svg = `<svg xmlns='http://www.w3.org/2000/svg' width='800' height='500' viewBox='0 0 800 500'>
    <rect width='800' height='500' fill='#7D8590'/>
    <g opacity='0.55' stroke='#F5F5F5' stroke-width='1'>
    ...
    <circle cx='400' cy='250' r='70' fill='none' stroke='#F5F5F5' stroke-width='2.5'/>
    <circle cx='400' cy='250' r='6' fill='#F5F5F5'/>
    <path d='M400 180 L400 120 M400 320 L400 380 M330 250 L270 250 M470 250 L530 250' stroke='#F5F5F5' stroke-width='2.5'/>
    <text x='400' y='260' font-family='monospace' font-size='22' fill='#F5F5F5' text-anchor='middle'>${safe}</text>
  </svg>`;
...
export function themeVars(theme) {
  return {
    ...
    '--accent': theme.accent, '--accent-text': theme.accentText,
  };
}

/* Picks a sensible starting theme based on the visitor's OS light/dark preference,
   so the site feels alive and responsive on first visit rather than always
   defaulting to one fixed look. Falls back to 'mono' if unavailable (e.g. SSR). */
...
/* =========================================================================
   FONTS
   Independent of color theme -- a person can pair any font with any theme.
   ========================================================================= */

export const FONTS = {
  grotesk: { label: 'Grotesk', sample: 'Aa', display: "'Space Grotesk', sans-serif", body: "'Inter', sans-serif" },
  poppins: { label: 'Poppins', sample: 'Aa', display: "'Poppins', sans-serif", body: "'Poppins', sans-serif" },
  editorial: { label: 'Editorial', sample: 'Aa', display: "'Playfair Display', serif", body: "'Inter', sans-serif" },
  manrope: { label: 'Manrope', sample: 'Aa', display: "'Manrope', sans-serif", body: "'Manrope', sans-serif" },
};
export const FONT_ORDER = ['grotesk', 'poppins', 'editorial', 'manrope'];

export function fontVars(fontKey) {
  const f = FONTS[fontKey] || FONTS.grotesk;
  return { '--font-display': f.display, '--font-body': f.body };
}

export function FontSwitcher({ fontKey, setFontKey }) {
  ...
  return (
    <>
      <button
        ...
        data-cursor-hover
        data-cursor-text="Font"
      >
        <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 15 }}>Aa</span>
      </button>
      {open && (
        <div className="theme-panel" style={{ left: 78 }}>
          {FONT_ORDER.map((key) => {
            const f = FONTS[key];
            return (
              <button
                key={key}
                className={`theme-option ${fontKey === key ? 'active' : ''}`}
                onClick={() => { setFontKey(key); setOpen(false); }}
              >
                <span style={{ fontFamily: f.display, fontWeight: 700, fontSize: 14, width: 18, textAlign: 'center', flexShrink: 0 }}>Aa</span>
                {f.label}
              </button>
            );
          })}
...
export const GlobalStyle = () => (
  <style>{`
    @import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@500;600;700&family=Inter:wght@400;500;600;700&family=IBM+Plex+Mono:wght@400;500;600&family=IBM+Plex+Sans+Condensed:wght@500;600;700&family=Poppins:wght@400;500;600;700&family=Playfair+Display:wght@500;600;700&family=Manrope:wght@400;500;600;700&display=swap');
    ...
    .mecha-root, .mecha-root .panel, .mecha-root .panel-2, .mecha-root .card, .mecha-root .btn,
    .mecha-root .input, .mecha-root .chip, .mecha-root .border-hair, .mecha-root .border-t-hair,
    .mecha-root .border-b-hair {
    ...
    .mecha-root .input::placeholder, .mecha-root textarea::placeholder { color: var(--text-muted); opacity: 0.7; }
    .mecha-root select.input { color: var(--text); }
    .mecha-root select.input option { background: var(--panel); color: var(--text); }
    .mecha-root .label {
    ...
    /* ---- Counters ---- */
    .stat-number { font-family: var(--font-display); font-weight: 700; font-variant-numeric: tabular-nums; }
...
/* Software / data field: a connected node graph */
export function NetworkArt() {
  const accent = { stroke: 'var(--accent)' };
  const accentFill = { fill: 'var(--accent)' };
  const nodes = [
    [210, 150], [90, 90], [330, 80], [80, 260], [340, 250], [210, 320], [150, 200], [270, 210],
  ];
  const edges = [[0, 1], [0, 2], [0, 6], [0, 7], [6, 3], [7, 4], [0, 5], [3, 5], [4, 5]];
  return (
    <svg viewBox="0 0 420 420" style={{ width: '100%', maxWidth: 420 }}>
      <g opacity="0.4" stroke="#2A2A2A" strokeWidth="1">
        <line x1="0" y1="105" x2="420" y2="105" /><line x1="0" y1="210" x2="420" y2="210" /><line x1="0" y1="315" x2="420" y2="315" />
      ...
      {edges.map(([a, b], i) => (
        <line key={i} x1={nodes[a][0]} y1={nodes[a][1]} x2={nodes[b][0]} y2={nodes[b][1]} style={accent} strokeWidth="1.3" className="trace-anim" opacity="0.7" />
      ))}
      {nodes.map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r={i === 0 ? 9 : 5} style={accentFill} className={i % 2 === 0 ? 'pulse' : ''} opacity={i === 0 ? 1 : 0.85} />
      ))}
      <circle cx="210" cy="150" r="30" fill="none" style={accent} strokeWidth="1.5" opacity="0.6" />
      <text x="210" y="154" textAnchor="middle" fontFamily="IBM Plex Mono, monospace" fontSize="10" style={accentFill} opacity="0.9">{'{ }'}</text>
    </svg>
  );
}

/* Design / creative field: overlapping abstract layers */
export function LayersArt() {
  const accent = { stroke: 'var(--accent)' };
  const accentFill = { fill: 'var(--accent)' };
  return (
    <svg viewBox="0 0 420 420" style={{ width: '100%', maxWidth: 420 }}>
      <g opacity="0.35" stroke="#2A2A2A" strokeWidth="1">
        <line x1="0" y1="105" x2="420" y2="105" /><line x1="0" y1="210" x2="420" y2="210" /><line x1="0" y1="315" x2="420" y2="315" />
        <line x1="105" y1="0" x2="105" y2="420" /><line x1="210" y1="0" x2="210" y2="420" /><line x1="315" y1="0" x2="315" y2="420" />
      </g>
      <rect x="110" y="110" width="150" height="150" rx="10" fill="none" style={accent} strokeWidth="1.6" opacity="0.55" transform="rotate(-8 185 185)" />
      <rect x="150" y="150" width="150" height="150" rx="10" fill="none" style={accent} strokeWidth="1.6" opacity="0.75" transform="rotate(6 225 225)" />
      <rect x="130" y="190" width="150" height="150" rx="10" fill="none" style={accent} strokeWidth="2" />
      <circle cx="205" cy="265" r="5" style={accentFill} className="pulse" />
      <path d="M205 265 L205 320 L120 320" fill="none" style={accent} strokeWidth="1.4" className="trace-anim" opacity="0.7" />
      <circle cx="120" cy="320" r="4" style={accentFill} className="pulse" />
      <path d="M280 220 L340 220 L340 130" fill="none" style={accent} strokeWidth="1.4" className="trace-anim" opacity="0.7" />
      <circle cx="340" cy="130" r="4" style={accentFill} className="pulse" />
    </svg>
  );
}

/* General / business field: a clean, safe abstract orbit motif */
export function OrbitArt() {
  ...
  return (
    <svg viewBox="0 0 420 420" style={{ width: '100%', maxWidth: 420 }}>
      ...
      <circle cx="210" cy="210" r="140" fill="none" style={accent} strokeWidth="1" opacity="0.35" />
      <circle cx="210" cy="210" r="95" fill="none" style={accent} strokeWidth="1.3" opacity="0.55" />
      <circle cx="210" cy="210" r="50" fill="none" style={accent} strokeWidth="1.8" />
      <circle cx="210" cy="210" r="7" style={accentFill} className="pulse" />
      <circle cx="350" cy="210" r="6" style={accentFill} />
      <circle cx="305" cy="105" r="5" style={accentFill} opacity="0.85" />
      <circle cx="115" cy="315" r="5" style={accentFill} opacity="0.85" />
      <path d="M210 160 L210 90" fill="none" style={accent} strokeWidth="1.3" className="trace-anim" opacity="0.7" />
      <circle cx="210" cy="90" r="4" style={accentFill} className="pulse" />
    </svg>
  );
}

export const HERO_ARTS = {
  engineering: { label: 'Engineering', Art: SchematicArt },
  software: { label: 'Software / Data', Art: NetworkArt },
  design: { label: 'Design / Creative', Art: LayersArt },
  general: { label: 'General', Art: OrbitArt },
};
export const HERO_ART_ORDER = ['engineering', 'software', 'design', 'general'];

export function Hero({ profile, onNavigate, reducedMotion }) {
  const artRef = useRef(null);
  useParallax(artRef, 0.1, reducedMotion);
  const HeroArtComp = (HERO_ARTS[profile.heroArt] || HERO_ARTS.engineering).Art;
  return (
      ...
      <div style={{ maxWidth: 1120, margin: '0 auto', position: 'relative' }} className="grid md:grid-cols-2 gap-12 items-center">
        <div className="fade-up">
          <div className="flex items-center gap-4 mb-5">
            ...
            <div className="eyebrow"><span className="dot" />{profile.role || 'Mechatronics Engineer'}</div>
          </div>
  ...
  return (
    ...
    <TiltCard className="card" style={{ cursor: 'pointer' }}>
      <div data-cursor-hover data-cursor-text={project.videoUrl ? 'Play' : 'View'} onClick={() => onOpen(project)}>
        <div style={{ position: 'relative', aspectRatio: '16/10', overflow: 'hidden', background: 'var(--panel-2)' }}>
          {thumb ? <img src={thumb} alt={project.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : null}
          {project.videoUrl ? (
...
export function ProfileTab({ profile, onSave }) {
  ...
  const handlePhoto = async (e) => {
    ...
    try { const dataUrl = await compressImage(file, 500, 0.8); setDraft((d) => ({ ...d, photo: dataUrl })); } catch (err) { console.error(err); }
  ...
  return (
    <div>
      ...
      <div className="panel-2 rounded-lg p-5 mt-6">
        <div className="font-mono mb-1" style={{ fontSize: 12, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Photo &amp; CV details</div>
        <p className="text-muted mb-4" style={{ fontSize: 12, lineHeight: 1.6 }}>
          Your photo appears gently next to your name on your portfolio, and is included in your Europass and Gulf CV downloads (the ATS-friendly format omits it, by design). The fields below are used only in CV downloads.
        </p>
        <div className="flex items-center gap-4 mb-4">
          {draft.photo && <img src={draft.photo} alt="" style={{ width: 64, height: 64, objectFit: 'cover', borderRadius: '50%', border: '1px solid var(--border)' }} />}
          <button className="btn btn-outline" onClick={() => photoRef.current.click()} disabled={uploadingPhoto}>
            {uploadingPhoto ? <Loader2 size={14} className="animate-spin" /> : <Upload size={14} />} {uploadingPhoto ? 'Processing...' : draft.photo ? 'Change photo' : 'Upload photo'}
          </button>
          {draft.photo && (
            <button className="btn btn-outline" onClick={() => setDraft((d) => ({ ...d, photo: '' }))}>Remove</button>
          )}
          <input ref={photoRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={handlePhoto} />
        </div>
        <div className="grid md:grid-cols-2 gap-x-6">
          {field('dateOfBirth', 'Date of birth (e.g. 12 March 1998)')}
          {field('nationality', 'Nationality')}
          {field('maritalStatus', 'Marital status')}
          {field('languages', 'Languages (e.g. "English — Fluent, Arabic — Basic")')}
        </div>
      </div>

      <div className="panel-2 rounded-lg p-5 mt-4">
        <div className="font-mono mb-1" style={{ fontSize: 12, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Portfolio appearance</div>
        <p className="text-muted mb-4" style={{ fontSize: 12, lineHeight: 1.6 }}>
          Pick the animated illustration shown on your homepage — choose whichever best fits your field.
        </p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {HERO_ART_ORDER.map((key) => {
            const opt = HERO_ARTS[key];
            const ArtPreview = opt.Art;
            return (
              <button
                key={key}
                onClick={() => setDraft((d) => ({ ...d, heroArt: key }))}
                className="rounded-lg p-3"
                style={{
                  border: `1px solid ${draft.heroArt === key ? 'var(--accent)' : 'var(--border)'}`,
              ...
              >
                <div style={{ width: '100%', height: 70, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                  <div style={{ width: 130, height: 130, flexShrink: 0 }}><ArtPreview /></div>
                </div>
                <div className="font-mono mt-2" style={{ fontSize: 10.5, color: draft.heroArt === key ? 'var(--accent)' : 'var(--text-muted)' }}>{opt.label}</div>
              </button>
            );
          })}
        </div>
      ...
      <button className="btn btn-primary mt-6" onClick={save} disabled={saving}>
      ...
      <div className="flex flex-col gap-3">
        {draft.map((p) => (
          <div key={p.id} className="panel-2 rounded-lg p-4 flex items-center gap-4">
            <img src={p.image} alt={p.title} style={{ width: 64, height: 44, objectFit: 'cover', borderRadius: 6, flexShrink: 0 }} />
